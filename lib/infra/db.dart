...
class DB {
  ...
  (uuid(), 'Receivable', 1, strftime('%Y-%m-%dT%H:%M:%S','now'), strftime('%Y-%m-%dT%H:%M:%S','now'), NULL);""",
    ...
    if (migrationVersion < 6) {
      ...
      db.execute(
        'CREATE TABLE IF NOT EXISTS budget_history (id TEXT PRIMARY KEY, budget_id TEXT NOT NULL REFERENCES budgets (id) ON DELETE CASCADE, usage REAL NOT NULL, threshold REAL NOT NULL, "limit" REAL NOT NULL, start_at TEXT, end_at TEXT)',
      );
