...
class NumericFormatter extends PatternFormatter {
  ...
  NumericFormatter({
    ...
    assert(
      this.thousandSeparator != this.separator,
  ...
  ///
  /// Converts the value into [double], ensuring only valid characters are retained.
  ///
  ...
  String format(TextEditingValue oldValue, TextEditingValue newValue) {
    ...
    /// Need convert to valid double value
    String decimals = value.replaceAll(comma, dot);

    final inputdoubles = decimals.split(dot);

    /// number in fraction need to convert base on the input length
    /// With fractionDigits = 3, Input = 100.3
    ///
    /// Bad result = 100.300
    int digits = allowFraction && inputdoubles.length > 1
        ? min(inputdoubles[1].length, fractionDigits ?? maxDigits)
        : 0;

    if (digits > 0) {
      decimals =
          '${inputdoubles[0]}${_getdoubleSep()}${inputdoubles[1].substring(0, digits)}';
    }
  ...
  TextEditingValue checkValidValue(
    ...
    final newEditingValue = filteringInputFormatter.formatEditUpdate(
      oldValue,
      newValue,
    );

    /// Remove the seconds fraction separator
    RegExp regExp = _getdoubleReg();
    final dotLength = regExp.allMatches(newEditingValue.text).length;
    if (dotLength <= 1) {
      return newEditingValue;
    } else {
      return newEditingValue.copyWith(
        text: newEditingValue.text.substring(
          0,
          newEditingValue.text.length - 1,
        ),
      );
    }
  }
  ...
  ///
  /// Returns the original [double] value or `null` if parsing fails.
  @override
